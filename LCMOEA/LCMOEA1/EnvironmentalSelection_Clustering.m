function [Population,z,znad] = EnvironmentalSelection_Clustering(Population,N,z,znad,cSize,a)
%Clustering-based environmental selection considering constraints
...
%--------------------------------------------------------------------------
    ...
    remainSize = floor((1.9-0.9*a)*N);
    if(a < beta)
        %remainSize = floor(1.05*N);
        [FrontNo,MaxFNo] = NDSort(Population.objs,remainSize);
    else
        %remainSize = floor(1.0*N);
